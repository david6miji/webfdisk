...
var RemoveAllBuildContainers = function RemoveAllBuildContainers(allDone) {
...
var build_create = function build_create() {
	var build = {};
...
R.route('/create').post(function (req, res, next) {
	...
	if (!buildOption) {
		SendError(res, 400, '빌드에 필요한 파라메터가 없습니다');return;
	}

	// 빌드 옵션 프러퍼티 존재 체크 
	if (typeof buildOption.name === "undefined") {
		SendError(res, 400, '빌드에 필요한 name 파라메터가 없습니다.');return;
	}
	if (typeof buildOption.env_id === "undefined") {
		SendError(res, 400, '빌드에 필요한 env_id 파라메터가 없습니다.');return;
	}
	if (typeof buildOption.src_id === "undefined") {
		SendError(res, 400, '빌드에 필요한 src_id 파라메터가 없습니다.');return;
	}
	...
	if (typeof build_env === "undefined") {
		SendError(res, 400, '빌드에 필요한 env_id 값이 잘못되었습니다.');return;
	}
	if (typeof build_src === "undefined") {
		SendError(res, 400, '빌드에 필요한 src_id 값이 잘못되었습니다.');return;
	}
	...
	var volumeBind = [build.env.toolchain + ':/toolchain/', build.env.kernel_src + ':/kernel_src/', build.env.kernel_build + ':/kernel_build/', build.src.path + ':/work/'];
...
R.route('/:id').get(function (req, res, next) {
	...
	if (typeof build === "undefined") {
		SendError(res, 404, 'ID 에 해당하는 빌드 정보가 없습니다.');return;
	}
...
R.route('/:id/clean').post(function (req, res, next) {
	...
	build.container.exec(options, function (err, exec) {
		...
		exec.start(function (err, stream) {
			if (err) {
				SendError(res, 500, '빌드 클린에 실패 했습니다.');return;
			}
...
R.route('/:id/start').post(function (req, res, next) {
	...
	build.container.exec(options, function (err, exec) {
		...
		exec.start(function (err, stream) {
			if (err) {
				SendError(res, 500, '빌드 시작에 실패 했습니다.');return;
			}
...
R.route('/:id/stop').post(function (req, res, next) {
	...
	build.container.exec(options, function (err, exec) {
		if (err) if (err) {
			SendError(res, 500, '빌드 중지에 실패 했습니다.');return;
		}
		exec.start(function (err, stream) {
			//			if (err) return;
